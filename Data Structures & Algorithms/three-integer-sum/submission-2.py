class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        '''
        - we are given a list made up of integers.
        - we need to return a list of all triplets, where
          the sum = 0.
        - the indice of an individual item in the triplet must be unique.
        - no duplicate triplets are allowed.
        - order doesn't matter.
        - according to the constraints the length of nums will be at least 3
        ...
        - we move j and k inward.
        - we do our second check involving j.
        - we do our third check involving k.
        
        checks breakdown that confused me:

        check 1: if i > 0 and nums[i] == nums[i - 1]:
                      continue

        - once i is greater than 0, meaning we are at the second item then
          we can now check whether the second item is equal to the previous 
          item and if they are then we can continue to the next item. This would
          allow us to skip duplicates before we even instantiate pointer j and k.

        check 2: while j < k and nums[j] == nums[j - 1]:
                      j += 1
        ...
        - once we found our triplet we have already moved j inward by 1 shown 
          ...
          make sure j stays below k and then we can check whether the current 
          value of j is equal to the past value of j, and if so we move j inward
          again.

        check 3: while j < k and nums[k] == nums[k + 1]:
                      k -= 1

        - once we found our triplet we have already moved k inward by 1 shown
          in the code when we found a triplet which was before the check, so now we
          can check for duplicates, but before we do that we must make sure j stays
          below k, then now we can check whether the current value of k is equal to the
        ...
        - The checks are very important in this code, and allow for a better flow
        ...
        Time Complexity: O(n^2) because of the for loop and the while loop inside,
        which dominates O(n log n).

        Space Complexity: O(1) or O(n) for the sort depending on the algorithm,
        O(m) for the result where m is the number of triplets.
        '''
        ...
        for i in range(len(nums) - 2):
            # check 1: skip duplicate i values
            ...
            while j < k:
                ...
                if total < 0:
                    ...
                    j += 1
                    k -= 1
                    # check 2: skip duplicate j values
                    while j < k and nums[j] == nums[j - 1]:
                        j += 1
                    # check 3: skip duplicate k values
